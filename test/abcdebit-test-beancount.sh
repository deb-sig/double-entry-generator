...
"$ROOT_DIR/bin/double-entry-generator" translate \
    ...
    --output "$ROOT_DIR/test/output/test-abcdebit-output.beancount" \
    "$ROOT_DIR/example/abcdebit/example-abcdebit-records.csv"

diff -u --color \
    "$ROOT_DIR/example/abcdebit/example-abcdebit-output.beancount" \
    "$ROOT_DIR/test/output/test-abcdebit-output.beancount"
