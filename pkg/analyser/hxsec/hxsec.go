...
type Hxsec struct { // Renamed from Htsec
}

func (h Hxsec) GetAllCandidateAccounts(cfg *config.Config) map[string]bool { // Renamed receiver
	...
	for _, r := range cfg.Hxsec.Rules { // Use Hxsec
...
func (h Hxsec) GetAccountsAndTags(o *ir.Order, cfg *config.Config, target, provider string) (bool, string, string, map[ir.Account]string, []string) { // Renamed receiver
	ignore := false
