...
func (AbcDebit) GetAllCandidateAccounts(cfg *config.Config) map[string]bool {
	uniq := make(map[string]bool)

	if cfg.AbcDebit == nil || len(cfg.AbcDebit.Rules) == 0 {
	...
	for _, rule := range cfg.AbcDebit.Rules {
...
func (AbcDebit) GetAccountsAndTags(order *ir.Order, cfg *config.Config, target, provider string) (bool, string, string, map[ir.Account]string, []string) {
	ignore := false
