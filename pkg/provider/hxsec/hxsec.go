...
func (h *Hxsec) fieldsToIR(fields []string) error {
	...
	// Parse common fields and handle transaction types
	if len(fields[colTradeTime]) == 0 {
		fields[colTradeTime] = "23:59:59"
	}
	// seems like 00:00:00 represents the end of the day
	if fields[colTradeTime] == "00:00:00" {
		fields[colTradeTime] = "23:59:59"
	}
	...
	// Add trade time universally if available
	if len(fields[colTradeTime]) > 0 {
	...
	case "指定交易":
		// This transaction type doesn't generate ledger entries.
		return nil
	default:
